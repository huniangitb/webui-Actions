...
const loadConfigs = async () => {
    ...
    if (pkgNames.length === 0) {
        ruleList.innerHTML = '<div class="p-5 text-center text-muted small">暂无已配置的应用</div>';
        return;
    }
    ...
    try {
        ...
        ruleList.innerHTML = `<div class="text-danger p-3 small">加载应用信息失败</div>`;
    }
    refreshMDB();
};
...
document.getElementById('btnOpenAppPicker').onclick = async () => {
    ...
    try {
        ...
        listContainer.innerHTML = `<div class="text-danger p-3 small">加载失败</div>`;
    }
};
