        ...
        if (regContent) {
            regContent.split('\n').forEach(line => {
                ...
                if (parts.length >= 2 && !line.trim().startsWith('#')) {
                    // 修复：参数可能是多个词（例如 MONITOR MERGE），需要拼接
...
const renderAppList = () => {
    ...
    listEl.innerHTML = items.length ? items.map(app => {
        ...
        if (param.includes('MONITOR')) {
            badgeClass = 'badge-warning';
        } else if (param.includes('PASSTHROUGH')) {
            badgeClass = 'badge-success';
        }
        
        if (param.includes('MERGE')) {
            badgeText += ' (M)'; // Add indicator for Merge
            if (!param.includes('MONITOR') && !param.includes('PASSTHROUGH')) {
                 badgeStyle = 'style="background:#6f42c1"'; // Purple for pure merge
            }
        }
