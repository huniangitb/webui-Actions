...
import { 
    mdiMathLog, mdiViewGridOutline, mdiShieldCheckOutline, mdiMonitorDashboard,
...
const ICONS = {
    ...
    GLOBAL: getSvg(mdiShieldCheckOutline),
    IO: getSvg(mdiMonitorDashboard),
    LOG: getSvg(mdiMathLog), // Specified Log icon
...
const showToast = (msg) => {
    try { toast(msg); } catch(e) {} // kernelsu native toast
    // Web UI toast fallback
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Theme Toggles
    document.getElementById('btnThemeToggleMobile').onclick = toggleTheme;
    document.getElementById('btnThemeToggleDesktop').onclick = toggleTheme;
    ...
    logViewer.parentElement.addEventListener('scroll', () => {
        if (logSelect.value === 'internal' && logViewer.parentElement.scrollTop + logViewer.parentElement.clientHeight >= logViewer.parentElement.scrollHeight - 50) fetchSysLogs();
    });
    ...
    document.getElementById('btnSaveAppConfig').onclick = async () => {
        try {
            ...
            if (!isEnabled) injectorStates.set(exactKey, 'OFF');
            else injectorStates.set(exactKey, 'ON');
...
const setupModeToggle = (groupName, visualId, rawId, contentId, parseFunc, genFunc) => {
    document.querySelectorAll(`button[name="${groupName}"]`).forEach(btn => {
        btn.onclick = () => {
            ...
            btn.classList.add('active');
            const isVisual = btn.dataset.mode === 'visual';
            if (isVisual) {
    ...
    try {
        ...
        if (injectorConf) {
            ...
            injectorConf.split('\n').forEach(line => {
                ...
                if (secMatch) {
                    ...
                    if (!injectorRulesMap.has(currentSection)) injectorRulesMap.set(currentSection, []);
        ...
        for (const uid of activeUsers) {
            ...
            if (lsRes) {
                ...
                for (const file of files) { // simplified for demo
        ...
        if (!Array.isArray(infos) || infos.length === 0) {
            ...
            if (fallbackList) fallbackList.split('\n').forEach(line => {
                ...
                if (trimLine && !trimLine.startsWith('#') && trimLine.includes('=')) {
                    ...
                    if (pkg) infos.push({ packageName: pkg, appLabel: trimLine.substring(firstEq + 1).trim() || pkg, isSystem: false });
                }
            });
        }
...
const renderAppList = () => {
    ...
    const html = items.map(app => {
        ...
        if (inj) {
            ...
            injStr = `<span style="font-size:10px;margin-left:6px;padding:2px 6px;background:var(--mx-s3);border-radius:4px;font-family:var(--mx-font-mono);">PID ${inj.pid} ${flags.join(' ')}</span>`;
        }

        return `
        <div class="app-item" onclick="openAppConfig('${app.packageName}')">
            ...
            <div class="app-info">
                <div class="app-name">${app.appLabel}${injStr}</div>
...
const addRuleRow = (type, target, source, containerId) => {
    ...
    div.innerHTML = `
        <select class="mx-select rule-type" style="width:100px; padding:8px 24px 8px 8px; font-size:12px;">
            <option value="REDIRECT">重定向</option><option value="HIDE">隐藏</option><option value="RO">只读</option><option value="ALLOW">豁免</option>
        </select>
        <div class="rule-inputs">
            <input type="text" class="mx-input rule-target" style="padding:6px; background:var(--mx-s1); border-radius:6px; font-size:12px;" placeholder="原始路径" value="${target}">
            <input type="text" class="mx-input rule-source ${type!=='REDIRECT'?'hidden':''}" style="padding:6px; background:var(--mx-s1); border-radius:6px; font-size:12px;" placeholder="重定向至" value="${source}">
        </div>
        <button class="mx-btn-icon btn-del">${ICONS.DELETE}</button>`;
...
window.openAppConfig = (pkg) => {
    ...
    const app = appMap.get(pkg); if (!app) return;
    ...
    tabsContainer.innerHTML = activeUsers.map(uid => `<button class="${uid === activeUsers[0] ? 'active' : ''}" data-uid="${uid}" onclick="window.switchAppUser(${uid})">User ${uid}</button>`).join('');
...
window.switchAppUser = (uid) => {
    ...
    document.querySelectorAll('#appUserTabs button').forEach(btn => btn.classList.toggle('active', parseInt(btn.dataset.uid) === uid));
    ...
    try {
        ...
        if (!res) ioState.hasMore = false;
            ...
            if (lastLine.startsWith('DONE|')) {
                const remain = parseInt(lastLine.split('|')[2]); ioState.hasMore = !isNaN(remain) && remain > 0; dataLines = lines.slice(0, -1);
            ...
            else if (ioState.offset === 0) document.getElementById('ioLogList').innerHTML = '<div style="padding:40px;text-align:center;color:var(--mx-t2);">暂无监控数据</div>';
        }
    } catch (e) { ioState.hasMore = false; }
    finally { ioState.loading = false; document.getElementById('ioLoadingIndicator').classList.add('hidden'); }
};
...
const renderIoRows = (lines) => {
    ...
    const html = lines.map(line => {
        ...
        let timeStr = "--:--:--";
        if (/^\d+$/.test(parts[0])) { const d = new Date(parseInt(parts[0])*1000); if(!isNaN(d)) timeStr = d.toLocaleTimeString('zh-CN', {hour12:false}); }
        else timeStr = parts[0].slice(0,8);
...
const fetchSysLogs = async () => {
    ...
    if (source === 'zygisk') {
        ...
        viewer.parentElement.scrollTop = viewer.parentElement.scrollHeight; return;
    }
    ...
    try {
        ...
        if (!res) sysState.hasMore = false;
            ...
            const lines = res.split('\n'); let dataLines = lines; const lastLine = lines[lines.length - 1];
            if (lastLine.startsWith('DONE|')) { sysState.hasMore = parseInt(lastLine.split('|')[2]) > 0; dataLines = lines.slice(0, -1); }
...
const checkStatus = async () => {
    try {
        ...
        ['Mobile', 'Desktop'].forEach(suffix => {
            ...
            if(badge && btn) {
                if (currentPid) {
                    badge.className = "mx-badge mx-badge-success"; badge.textContent = "RUNNING";
                    btn.innerHTML = ICONS.STOP;
                } else {
                    badge.className = "mx-badge mx-badge-gray"; badge.textContent = "STOPPED";
...
const toggleStatus = async () => {
    if (currentPid) { await run(`kill -15 ${currentPid}`); showToast("发送停止信号..."); }
    else { await run(`sh ${SERVICE_SH}`); showToast("启动服务..."); setTimeout(loadData, 1000); }
...
const parseIgnoreToVisual = (text) => { const c = document.getElementById('ignoreBuilderContainer'); c.innerHTML=''; if(text) text.split('\n').forEach(l=>{const v=l.trim(); if(v&&!v.startsWith('#')) addIgnoreRow(v);}); if(c.children.length===0) addIgnoreRow(''); };
...
const addIgnoreRow = (path) => {
    const div = document.createElement('div'); div.className='rule-row';
    div.innerHTML = `<input type="text" class="mx-input" style="background:var(--mx-s1); border-radius:6px; font-size:12px; padding:6px;" placeholder="要忽略的路径前缀" value="${path}"><button class="mx-btn-icon btn-del">${ICONS.DELETE}</button>`;
...
const setupAutocomplete = (input) => {
    ...
    input.addEventListener('input', debounce(async (e) => {
        const val = e.target.value; let pDir=PATH_PREFIX_REAL+'/', sPre=""; let dBase="/";
        ...
        try {
            ...
            box.innerHTML = sugs.map(s=>`<div class="suggestion-item" onmousedown="event.preventDefault()" onclick="window._currentInput.value='${s.t}';window._currentInput.dispatchEvent(new Event('input'))"><span>${s.i}</span><span style="overflow:hidden;text-overflow:ellipsis;">${s.t}</span></div>`).join('');
            
            const rect = input.getBoundingClientRect(); box.style.width=rect.width+'px'; box.style.left=rect.left+'px'; box.style.top=(rect.bottom+4)+'px'; box.style.display='block';
