...
const renderAppList = () => {
    ...
    listEl.innerHTML = items.length ? items.map(app => {
        ...
        if (app.boundParam === 'MONITOR') {
            badgeClass = 'badge-warning';
        } else if (app.boundParam === 'PASSTHROUGH') {
            badgeClass = 'badge-success';
        } else if (app.boundParam && app.boundParam.includes('MERGE')) {
...
window.openAppConfig = (pkg) => {
    ...
    checkMerge.disabled = false;

    if (mode.includes('MONITOR')) {
        document.getElementById('modeMonitor').checked = true;
        checkMerge.disabled = true;
    } else if (mode.includes('PASSTHROUGH')) {
        document.getElementById('modePassthrough').checked = true;
        checkMerge.disabled = true;
    } else {
        document.getElementById('modeDefault').checked = true;
    }

    // Bind change events to toggle checkbox availability
    document.querySelectorAll('input[name="bindMode"]').forEach(el => {
        el.onchange = (e) => {
            checkMerge.disabled = e.target.value !== '';
        };
    });
...
document.getElementById('btnSaveBinding').onclick = async () => {
    ...
    let mode = document.querySelector('input[name="bindMode"]:checked')?.value || "";
    const isMerge = document.getElementById('checkMerge').checked;

    // Only apply MERGE if mode is Default (empty)
    if (mode === "" && isMerge) {
        mode = "MERGE";
    }
