...
// [修改] 移除FIXED_PRECISION, 因为所有值现在都是整数
const VALUE_MULTIPLIER = 100; // 用于写入节点/配置文件时的最终乘数

// [修改] 默认配置现在使用整数
const defaultConfig = {
    red: { intercept: 256, slope: 0 },
    green: { intercept: 256, slope: 0 },
    blue: { intercept: 256, slope: 0 },
};
...
// --- 轮询函数 (无修改) ---
async function pollSystemStatus() { try { const { stdout } = await exec('settings get system peak_refresh_rate'); const newRate = Math.round(parseFloat(stdout.trim())) || 60; if (newRate !== lastKnownRefreshRate) { lastKnownRefreshRate = newRate; currentRefreshRate = newRate; refreshRateValue.innerText = `${currentRefreshRate} Hz`; currentConfigPath = `${MODULE_PATH}/${currentRefreshRate}hz.config`; toast(i18next.t('toast.refreshRateChanged', { rate: newRate }), 'info'); await loadConfigAndRender(); } } catch (e) { /* 忽略错误 */ } try { if (wizardModal && wizardModal._isShown) return; const { stdout } = await exec(`cat ${BACKLIGHT_PATH}`); const newBrightness = parseInt(stdout.trim()); if (newBrightness !== lastKnownBrightness) { lastKnownBrightness = newBrightness; const percentage = Math.round(((newBrightness - 1) / (maxBrightness - 1)) * 100); brightnessValue.innerText = i18next.t('status.brightnessValue', { value: newBrightness, percent: percentage }); brightnessSlider.value = percentage; } } catch (e) { /* 忽略错误 */ } }

// --- 多语言UI更新 (无修改) ---
function updateUIText() { document.querySelectorAll('[data-i18n]').forEach(el => { const key = el.getAttribute('data-i18n'); const translation = i18next.t(key, { returnObjects: true }); if (typeof translation === 'object' && translation.icon && translation.text) { el.innerHTML = `${createIcon(icons[translation.icon])}${translation.text}`; } else { el.innerHTML = translation; } }); document.documentElement.lang = i18next.language; document.title = i18next.t('title'); document.querySelectorAll('.form-outline').forEach(formOutline => new Input(formOutline).update()); updateChart(); toggleAdvancedMode(isAdvancedMode, false); }

// --- UI模式切换 (无修改) ---
function toggleAdvancedMode(enable, showToast = true) { isAdvancedMode = enable; advancedConfigEditor.style.display = enable ? 'block' : 'none'; const key = enable ? 'buttons.advancedMode.exit' : 'buttons.advancedMode.enter'; const translation = i18next.t(key, { returnObjects: true }); if (typeof translation === 'object') { advancedModeButton.innerHTML = `${createIcon(icons[translation.icon])}${translation.text}`; } if (enable) { advancedModeButton.classList.remove('btn-secondary'); advancedModeButton.classList.add('btn-primary'); if (showToast) toast(i18next.t('toast.advancedMode.on'), 'info'); } else { advancedModeButton.classList.remove('btn-primary'); advancedModeButton.classList.add('btn-secondary'); if (showToast) toast(i18next.t('toast.advancedMode.off'), 'info'); } }

// --- 主题管理 (无修改) ---
function updateTheme() { const isDarkMode = window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches; document.documentElement.setAttribute('data-mdb-theme', isDarkMode ? 'dark' : 'light'); updateChart(); }

// --- 工具函数 (无修改) ---
const scaleToSystemBrightness = (percentage) => Math.max(1, Math.round(1 + (percentage / 100) * (maxBrightness - 1)));
async function setSystemBrightness(percentage) { if (!BACKLIGHT_PATH) return; const systemValue = scaleToSystemBrightness(percentage); try { await exec(`echo ${systemValue} > ${BACKLIGHT_PATH}`); if (!wizardModal || !wizardModal._isShown) { brightnessValue.innerText = i18next.t('status.brightnessValue', { value: systemValue, percent: percentage }); lastKnownBrightness = systemValue; } } catch (e) { toast(i18next.t('toast.saveFailed', { error: `Brightness: ${e.message}` }), 'error'); } }

// --- Chart.js 函数 (无修改) ---
function calculateChartData(params) { const labels = []; const colorData = { red: [], green: [], blue: [] }; for (let p = 1; p <= 100; p++) { labels.push(p); const systemBrightness = scaleToSystemBrightness(p); const log_b = Math.log(systemBrightness); for (const color in params) { const { intercept, slope } = params[color]; colorData[color].push(intercept + (slope * log_b)); } } const datasets = [{ label: i18next.t('params.red'), data: colorData.red, borderColor: 'rgba(255, 99, 132, 1)', backgroundColor: 'rgba(255, 99, 132, 0.2)', tension: 0.1, borderWidth: 2, pointRadius: 0 }, { label: i18next.t('params.green'), data: colorData.green, borderColor: 'rgba(75, 192, 192, 1)', backgroundColor: 'rgba(75, 192, 192, 0.2)', tension: 0.1, borderWidth: 2, pointRadius: 0 }, { label: i18next.t('params.blue'), data: colorData.blue, borderColor: 'rgba(54, 162, 235, 1)', backgroundColor: 'rgba(54, 162, 235, 0.2)', tension: 0.1, borderWidth: 2, pointRadius: 0 }]; return { labels, datasets }; }
function initChart() { if (colorChart) colorChart.destroy(); const isDarkMode = document.documentElement.dataset.mdbTheme === 'dark'; const tickColor = isDarkMode ? 'rgba(255, 255, 255, 0.7)' : 'rgba(0, 0, 0, 0.7)'; const gridColor = isDarkMode ? 'rgba(255, 255, 255, 0.1)' : 'rgba(0, 0, 0, 0.1)'; const chartData = calculateChartData(globalConfig); colorChart = new Chart(chartCanvas.getContext('2d'), { type: 'line', data: chartData, options: { responsive: true, maintainAspectRatio: false, scales: { x: { type: 'logarithmic', title: { display: true, text: i18next.t('status.brightness'), color: tickColor }, min: 1, max: 100, ticks: { color: tickColor, callback: function (value) { const shown_ticks = [1, 2, 5, 10, 20, 50, 100]; if (shown_ticks.includes(Number(value))) return value + '%'; }, generateTicks: function () { return [{ value: 1 }, { value: 2 }, { value: 5 }, { value: 10 }, { value: 20 }, { value: 50 }, { value: 100 }]; } }, grid: { color: gridColor } }, y: { title: { display: true, text: i18next.t('chart.yAxisTitle'), color: tickColor }, ticks: { color: tickColor }, grid: { color: gridColor } } }, plugins: { legend: { display: true, labels: { color: tickColor } } } } }); }
function updateChart() { if (!chartCanvas) return; if (colorChart) { const isDarkMode = document.documentElement.dataset.mdbTheme === 'dark'; const tickColor = isDarkMode ? 'rgba(255, 255, 255, 0.7)' : 'rgba(0, 0, 0, 0.7)'; const gridColor = isDarkMode ? 'rgba(255, 255, 255, 0.1)' : 'rgba(0, 0, 0, 0.1)'; colorChart.data = calculateChartData(globalConfig); colorChart.options.scales.x.title.text = i18next.t('status.brightness'); colorChart.options.scales.x.title.color = tickColor; colorChart.options.scales.x.ticks.color = tickColor; colorChart.options.scales.x.grid.color = gridColor; colorChart.options.scales.y.title.text = i18next.t('chart.yAxisTitle'); colorChart.options.scales.y.title.color = tickColor; colorChart.options.scales.y.ticks.color = tickColor; colorChart.options.scales.y.grid.color = gridColor; colorChart.options.plugins.legend.labels.color = tickColor; colorChart.update(); } else { initChart(); } }
...
async function applyKcal(params, useRefreshRate = currentRefreshRate) {
    ...
    try {
        const cmds = Object.entries(params).map(([color, { intercept, slope }]) => {
            // [修改] 直接使用整数值，然后乘以乘数
            const i = Math.round(intercept * VALUE_MULTIPLIER);
            const s = Math.round(slope * VALUE_MULTIPLIER);
...
function parseConfig(text) {
    ...
    // [修改] 读取到的值需要除以乘数，变回程序内部使用的值
    return {
        red: { intercept: ri / VALUE_MULTIPLIER, slope: rs / VALUE_MULTIPLIER },
        green: { intercept: gi / VALUE_MULTIPLIER, slope: gs / VALUE_MULTIPLIER },
        blue: { intercept: bi / VALUE_MULTIPLIER, slope: bs / VALUE_MULTIPLIER },
    };
}

function serializeConfig(params) {
    // [修改] 直接使用程序内部的整数值
    const ri = params.red.intercept;
    const rs = params.red.slope;
    const gi = params.green.intercept;
    const gs = params.green.slope;
    const bi = params.blue.intercept;
    const bs = params.blue.slope;
...
function renderUI(params) {
    for (const color in uiElements) {
        ...
        uiElements[color].slopeInput.value = slope;
...
async function saveConfig() { 
    // [修改] 保存时，直接将内部整数值拼接成字符串，然后由 applyKcal 负责乘100写入
    const configToSave = serializeConfig(globalConfig);
    const filename = currentConfigPath.split('/').pop(); 
    try { 
        // 写入配置文件的值，不再乘以100，因为内部已经是整数
        await exec(`echo '${configToSave}' > ${currentConfigPath}`);
...
function calculateFit(point1, point2) {
    ...
    // [修改] 返回整数
    return { intercept: Math.round(intercept), slope: Math.round(slope) };
}
...
function startWizard() {
    ...
    for (const stepKey in wizardControls) {
        const step = wizardControls[stepKey]; const data = wizardData[stepKey];
        for (const colorKey in step) {
            const control = step[colorKey]; const value = data[colorKey];
    ...
    wizardModal.show(); setSystemBrightness(wizardData.step1.brightnessPercent);
}
...
function handleWizardCancel() { applyKcal(originalConfigForWizard); wizardModal.hide(); toast(i18next.t('toast.wizardCancelled'), 'info'); }
...
function handleWizardColorPreview(stepNum) {
    ...
    const r = parseInt(controls.red.input.value); const g = parseInt(controls.green.input.value); const b = parseInt(controls.blue.input.value);
...
async function init() {
    ...
    for (const color in uiElements) {
        ...
        elements.slopeSlider.addEventListener('input', (e) => handleParamChange('slope', parseInt(e.target.value)));
        elements.slopeInput.addEventListener('change', (e) => { const val = parseInt(e.target.value) || 0; const clampedVal = Math.max(-50, Math.min(val, 50)); handleParamChange('slope', clampedVal); });
    }

    // [新增] 为所有步进按钮添加事件委托
    document.body.addEventListener('click', (e) => {
        if (e.target.matches('.btn-stepper')) {
    ...
    for (const stepNum of [1, 2]) {
        ...
        for (const colorKey in wizardControls[stepKey]) {
            ...
            input.addEventListener('change', () => {
                ...
                const clampedValue = Math.max(parseInt(input.min), Math.min(value, parseInt(input.max)));
                if (value !== clampedValue) input.value = clampedValue;
    ...
    wizardModalElement.addEventListener('hidden.mdb.modal', () => { if (brightnessBeforeWizard !== -1) { const originalPercentage = Math.round(((brightnessBeforeWizard - 1) / (maxBrightness - 1)) * 100); setSystemBrightness(originalPercentage); brightnessBeforeWizard = -1; } });
