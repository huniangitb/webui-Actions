...
export async function listDirectory(path: string): Promise<void> {
  ...
  try {
    ...
    fileListEl.innerHTML = `<div class="text-center text-muted p-4" style="color:var(--mx-red);">读取失败: ${msg}</div>`;
  }
}
